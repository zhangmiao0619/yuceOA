...
import { Card, Table, Button, Modal, Form, Input, Select, Space, message, Tag, DatePicker, Tabs, Popconfirm, Row, Col } from 'antd'
import { PlusOutlined, CheckOutlined, CloseOutlined } from '@ant-design/icons'
import dayjs from 'dayjs'
import { useAuthStore } from '../stores/auth'
import api from '../lib/api'

const TRANSPORT_OPTIONS = ['飞机', '高铁', '汽车', '其他']
...
export default function BusinessTripRequests() {
  ...
  const fetchRecords = async () => {
    setLoading(true)
    try {
      const res = await api.get('/hr-requests/business-trip')
      if (res.success) setRecords(res.data)
    } catch { message.error('获取出差记录失败') }
  ...
  const fetchPending = async () => {
    ...
    try {
      const res = await api.get('/hr-requests/admin/business-trip-pending')
  ...
  const handleCreate = async (values) => {
    try {
      ...
      const res = await api.post('/hr-requests/business-trip', payload)
      if (res.success) {
  ...
  const handleApprove = async (id, approved, notes) => {
    try {
      const res = await api.put(`/hr-requests/admin/business-trip/${id}/approve`, { approved, notes })
      if (res.success) {
  ...
  const columns = [
    ...
    { title: '出差目的', dataIndex: 'purpose', key: 'purpose', render: (v) => v || '-' },
    {
  ...
  return (
    <div>
      ...
      <Modal title="提交出差申请" open={modalVisible} onCancel={() => setModalVisible(false)} onOk={() => form.submit()} width={600}>
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          ...
          <Form.Item name="transport" label="交通工具" initialValue="汽车">
            <Select>
              {TRANSPORT_OPTIONS.map((t) => <Select.Option key={t} value={t}>{t}</Select.Option>)}
            </Select>
          </Form.Item>
          <Form.Item name="purpose" label="出差目的" rules={[{ required: true, message: '请输入出差目的' }]}>
            <Input.TextArea rows={3} placeholder="请输入出差目的" />
          </Form.Item>
        </Form>
      </Modal>
    </div>
